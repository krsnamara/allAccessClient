...
export default function MapToggle({ onToggle }) {
  ...
  return (
    <div className="checkbox-wrapper-8">
      <input type="checkbox" id="cb3-8" className="tgl tgl-skewed" checked={isChecked} onChange={handleToggle} />
      <label htmlFor="cb3-8" className="tgl-btn">
        <img src={isChecked ? Map : Grid } alt={isChecked ? 'Map' : 'Grid'} className="toggle-image" />
      </label>
    </div>
  );
}
